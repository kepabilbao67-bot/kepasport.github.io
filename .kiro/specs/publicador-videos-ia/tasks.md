...
- [x] 1. Declaraciones de plataforma y validación (sin tocar main.wasp)
  - [x] 1.1 Crear el mapa de plataformas `src/server/publisher/platforms.ts`
  ...
  - [x] 1.2 Crear la validación `src/server/publisher/validation.ts`
  ...
  - [x]* 1.3 Escribir prueba de propiedad para la adaptación del prompt y la validación
...
- [x] 2. Capa de IA: generación de contenido por plataforma
  - [x] 2.1 Crear `src/server/publisher/aiContent.ts`
  ...
  - [x]* 2.2 Escribir prueba de propiedad para `parseContenido` (round-trip / forma estable)
  ...
  - [x]* 2.3 Escribir prueba de propiedad para la guarda de clave de API
...
- [x] 3. Despachador de salida (publicación)
  - [x] 3.1 Crear `src/server/publisher/dispatch.ts`
  ...
  - [x]* 3.2 Escribir prueba de propiedad para deduplicación de destinos
    - **Property 12: Deduplicación de destinos de salida** — _Validates: Requirements 5.2, 8.4_
  - [x]* 3.3 Escribir prueba de propiedad para fan-out y resiliencia
...
- [x] 4. Checkpoint — Asegurar que pasan todas las pruebas
  - Ejecutar la suite de pruebas; asegurarse de que todo pasa y preguntar al usuario si surgen dudas.

- [x] 5. Consultas del backend
  - [x] 5.1 Crear `src/server/publisher/queries.ts`
  ...
  - [x]* 5.2 Escribir prueba de propiedad para aislamiento por propietario y orden
...
- [x] 6. Acciones del backend
  - [x] 6.1 Implementar `createVideoPost` en `src/server/publisher/actions.ts`
  ...
  - [x]* 6.2 Escribir prueba de propiedad para inicialización de contenidos y plataformas manuales
  ...
  - [x] 6.3 Implementar `generatePlatformContent` en `src/server/publisher/actions.ts`
  ...
  - [x]* 6.4 Escribir prueba de propiedad para regeneración y conservación ante error
  ...
  - [x] 6.5 Implementar `publishPlatformContent` y `markManual` en `src/server/publisher/actions.ts`
  ...
  - [x]* 6.6 Escribir prueba de propiedad para transiciones de estado y control de acceso
...
- [x] 7. Checkpoint — Asegurar que pasan todas las pruebas
  - Ejecutar la suite de pruebas; asegurarse de que todo pasa y preguntar al usuario si surgen dudas.

- [x] 8. Cableado en Wasp (main.wasp) — adiciones
  - [x] 8.1 Añadir entidades, relación y operaciones en `main.wasp`
  ...
  - [x] 8.2 Añadir rutas y páginas protegidas en `main.wasp`
    - Añadir `PublicacionesRoute`, `NuevaPublicacionRoute` y `DetallePublicacionRoute` con sus páginas y `authRequired: true`
    - _Requisitos: 1.1_

- [x] 9. Interfaz React (español)
  - [x] 9.1 Extender el catálogo i18n `src/client/i18n/es.ts`
  ...
  - [x]* 9.2 Escribir prueba por ejemplo para etiquetas de estado en español
  ...
  - [x] 9.3 Crear `src/client/publisher/PaginaPublicaciones.tsx` (listado)
  ...
  - [x] 9.4 Crear `src/client/publisher/FormularioPublicacion.tsx` (creación)
  ...
  - [x] 9.5 Crear `src/client/publisher/DetallePublicacion.tsx` (variantes por plataforma)
  ...
  - [x]* 9.6 Escribir pruebas por ejemplo de UI
...
- [x] 10. Integración y verificación final
  - [x]* 10.1 Escribir prueba de integración del flujo crear → generar → publicar
    - Mockear `streamCompletion` y `fetch`; verificar el ciclo completo con `fakeContext`/`mockContext`
    - _Requisitos: 2.1, 3.1, 5.1, 5.3_
  - [x] 10.2 Checkpoint final — Asegurar que pasan todas las pruebas
